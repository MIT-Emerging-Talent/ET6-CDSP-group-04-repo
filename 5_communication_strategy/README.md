# Communication Strategy

## 🌟 Goals & Target Audience

### Target Audience

- **Displaced Youth (ages 16–30) in the U.S.** looking for accessible IT career paths.
- **Nonprofit and government program coordinators** who support these youth.
- **Employers and recruiters** interested in identifying accessible entry-level talent pipelines.

### Capabilities and Constraints

- **Constraints:** Limited funding, unfamiliarity with the U.S. education system, lack of social capital, limited time
  due to survival priorities
- **Capabilities:** High motivation to work, foundational digital literacy, increasing smartphone and internet access

---

## 🎯 Communication Goals

- **Help displaced youth understand** the current challenges and realities of the U.S. IT job market in 2025, including
  high competition and the importance employers place on formal education
- **Present a realistic pathway hierarchy** based on research:
  traditional degrees → special programs → bootcamps → online/nonprofit courses
- **Encourage long-term investment thinking**, even if the timeline is 2–4 years, to increase chances of meaningful employment
- **Emphasize agency** by providing tools to help individuals navigate options based on their personal circumstances

---

## 👤 Persona Snapshot

**Name:** Ali  
**Age:** 22  
**Background:** Syrian refugee resettled in Houston, TX  
**Status:** Completed high school, speaks conversational English  
**Aspirations:** Become a software engineer but unsure where to start  
**Pain Points:** Confused by U.S. education system, can't afford college, overwhelmed by YouTube tutorials

---

## 📦 Chosen Formats & Platforms

### 1. **YouTube Explainer Video**

- **Length:** 3 minutes
- **Style:** Animated, narrative voiceover
- **Purpose:** Deliver a story-driven summary of findings and empower users to self-identify their path

### 2. **Instagram Carousel Campaign**

- **Slides:** 6–8
- **Purpose:** Convert flowchart insights into quick, swipable tips with CTAs and links to video/site

### 3. **Interactive Flowchart**

- **Tool:** Miro or Canva
- **Purpose:** Help individuals navigate options based on money/time via a “choose your own path” logic tree

### 4. **Website Landing Page**

- **Tool:** GitHub Pages or Carrd
- **Purpose:** A single link to host all artefacts (video, carousel, flowchart, form)

### 5. **Academic Paper (Optional)**

- **Tool:** Google Docs or Overleaf (LaTeX)
- **Purpose:** Present a formalized write-up of the research insights,
  suitable for academic or policy-oriented audiences;
- includes references, methodology, and limitations

---

## 🗺️ Storyboard / Script Outline

### Video Script (Scene-Based)

1. **Scene 1:** “How can you break into tech without a degree?”
2. **Scene 2:** Market in 2025 is tough. More candidates than jobs.
3. **Scene 3:** Employers often still prefer traditional degrees.
4. **Scene 4:** But not everyone has 4 years or $40,000.
5. **Scene 5:** Let’s find your path: What do you have—Time? Money? Both? Neither?
6. **Scene 6:** Walk through the flowchart visually.
7. **Scene 7:** Real examples (MIT ET, bootcamps, Coursera).
8. **Scene 8:** “Tech needs your voice. Start where you are. Choose your path.”

### Carousel Slides (Instagram)

1. **Title:** “No Degree? No Problem?”
2. **Reality Check:** “40% of entry-level jobs don’t require a CS degree—but many still do.”
3. **Path 1:** If you have time, but no money
4. **Path 2:** If you have money, but no time
5. **Path 3:** If you have neither
6. **Tool Link:** Explore your path in our free flowchart
7. **CTA:** Watch the full video to get started
8. **Encouragement:** “Start small. Think long. You belong in tech.”

---

## 📈 Engagement Plan

| Platform   | Metrics to Track                          |
|------------|--------------------------------------------|
| YouTube    | Views, avg watch time, comments            |
| Instagram  | Impressions, saves, shares, replies        |
| Website    | Bounce rate, time-on-page, scroll depth    |
| Feedback   | Google Form (3 questions, embedded on site)|

---

## ⚠️ Acknowledging Uncertainty

While our research indicates traditional degrees remain the most reliable pathway to IT careers, **there are no guarantees**
in a changing job market. Outcomes vary based on location, individual motivation, and economic shifts. We present these
options as **probability-informed paths**, not promises.

---

## 🎨 Tone & Accessibility

- Clear, conversational English with minimal jargon
- Alt text and subtitles for all visual media
- High contrast design for readability
- Mobile-first layout; future goal: multilingual captions (Arabic, Spanish)

---

## 🤔 Why This Format?

- **Video** builds narrative and trust
- **Instagram** meets our audience where they are
- **Flowchart** helps users make personalized decisions
- **Landing Page** consolidates artefacts + feedback form

---

## 🔗 Artefact Links (To be added)

- [ ] YouTube Video  
- [ ] Instagram Carousel  
- [ ] Flowchart (Canva or Miro)  
- [ ] Website Landing Page  
- [ ] Google Feedback Form  

---

## 📍 Submission Checklist

- [x] Strategy document  
- [ ] Communication artefact(s)  
- [ ] Feedback mechanism  
- [ ] Git tag created  
- [ ] Retrospective (group + individual)  
- [ ] Milestone 5 Survey completed
  